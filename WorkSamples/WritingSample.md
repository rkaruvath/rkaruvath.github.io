...
As an administrator, configure Agentic chat in BMC Helix ITSM. Agentic chat  provides the service desk agents with a conversational, AI-driven experience to search for information from knowledge articles when working on incidents. Agentic chat uses the advanced AI capabilities of BMC HelixGPT to augment the full text search-based global search with an AI-powered, chat-based search.
