...
|---|---|---|---|
|Create a change request|POST|/change|Creates a new change request.|
|Show a change request|GET|/change/{id}|Shows the details of a change request, by ID.|
|Update a change request|PATCH|/change/{id}|Updates a change request, by ID.
...
urgency (optional)|Urgency of the change based on its impact on the business processes.<br>Urgency is a measure of how long it will be until a change has a significant business impact.<br>For example, a high impact incident may have low urgency if the impact does not affect the business until the end of the financial year.<br>The following are the supported values for urgency:<br>- 1000: "1-Critical"<br>- 2000: "2-High"<br>- 3000: "3-Medium"<br>- (Default) 4000: "4-Low"|integer|
...
|priority (optional)|Priority identifies the relative importance of a change. Priority is based on impact and urgency, and it identifies required times for actions to be taken. <br>The following are the supported values for priority:<br>- 0: "1-Critical"<br>- 1: "2-High"<br>- 2: "3-Medium"<br>- (Default) 3: "4-Low"|integer|
|risk level (optional)|Risk level is the anticipated risk for the change from 5 (highest risk) to 1 (lowest risk).<br>For example, if the support team needs to install a critical security update on all the computers in the sales department during peak working hours, the risk level can be 4 or 5. If the critical security update can be installed during offline working hours, the risk level can be 2.<br>The following are the supported values for risk level:<br>- 1 : Minimal (lowest risk)<br>- 2 : Low<br>- 3 : Medium<br>- 4 : High<br>- 5 : Extreme (highest risk)|integer|
|locationCompany|The location where the change needs to be implemented.|string|
|requestedFor|The user who initiated the change request.|object{1}|
...
|Status code|Description|
|---|---|
|200 OK|Returned matching change object if the request is successful.|
...
|Name|Description|Type|
|---|---|---|
status (Optional)|Status is the state of change request in its control process flow and transitions as it follows the process through its lifecycle. Following are the supported values for status:<br>- (Default) 0: "Draft"<br>- 1: "Request For Authorization"<br>- 2: "Request For Change"<br>- 3: "Planning In Progress"<br>- 4: "Scheduled For Review"<br>- 5: "Scheduled For Approval"<br>- 6: "Scheduled"<br>- 7: "Implementation In Progress"<br>- 8: "Pending"<br>- 9: "Rejected"<br>- 10: "Completed"<br>- 11: "Closed"<br>- 12: "Cancelled"|integer|
|statusReason (Optional)|Reason of the current status change.|string|
|resolutionNote (Optional)|Information about the resolution|string|
